    ...
    while queue:
        ...
        for neighbor in get_valid_moves(game_map, current):
            if neighbor not in visited:
                ...
                # A ogni nodo espanso si aggiunge nel dizionario associandogli il nodo parent che sarebbe quello attuale
...
def a_star(game_map: np.ndarray, start: Tuple[int, int], target: Tuple[int, int], h: callable) -> List[Tuple[int, int]]:
    ...
    # Una coda prioritaria che conterrà i nodi ancora da esplorare, ordinati in base al valore f(n) = g(n) + h(n).
    open_list = PriorityQueue()
    # Una lista vuota che conterrà i nodi che sono già stati esplorati.
    close_list = []
    # additional dict which maintains the nodes in the open list for an easier access and check
    # Un dizionario che mantiene una mappa tra i nodi nella lista aperta e i loro valori g(n).
    support_list = {}
    ...
    # Un dizionario che tiene traccia dei genitori di ciascun nodo nella soluzione.
    parent = {start: None}
